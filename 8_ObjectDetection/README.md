...
By running ```part_a.py``` we load a Roboflow 2.0 model that has been trained to classify indoor scenes. We use said model to classify the images stored within ```img/grocerystore```. The images with classifications, as well as the raw data can be found in ```part_a_classifications```. 
...
By Running ```part_b.py``` we run both Yolov5s and Yolov8n on the same images as part a. The processed images with bounding boxes can be found within ```part_b_detections/yolo*/iimg```.
